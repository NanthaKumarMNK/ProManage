...
const registerUser = async (req, res, next) => {
    try {
        ...
        if (isExistingUser) {
            return res
                .status(400).json({ errorMessage: "User already exists" });
        }
...
const putUserData = async (req, res, next) => {
    try {
      ...
      if (formData.oldPassword && formData.newPassword) {
        const isPasswordMatch = await bcrypt.compare(formData.oldPassword, userToUpdate.password);
        if (!isPasswordMatch) {
          return res.status(400).json({ errorMessage: 'Invalid old password' });
        }
  
        if (formData.oldPassword === formData.newPassword) {
          return res.status(400).json({ errorMessage: 'New password must be different from old password' });
        }
  
        if (formData.newPassword.length < 4) {
          return res.status(400).json({ errorMessage: 'New password must be at least 4 characters long' });
        }
  ...
  try {
    ...
    if (userToUpdate.assignie.length === 0) {
      ...
      if (userToUpdate.assignie.includes(formData.assignie)) {
        return res.status(400).json({ errorMessage: "User already assigned to board" });
      }
      userToUpdate.assignie.push(formData.assignie);
    }
...
module.exports = { loginUser, registerUser,getUserData,putUserData,putUser,getAssignie};
