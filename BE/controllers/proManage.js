...
const postCreateList = async (req, res, next) => {
  try {
    ...
    if (
      Object.keys(formData.list).length !==
      Object.keys(formData.checkList).length
    ) {
      return res
        .status(400)
        .json({ errormessage: "Invalid check list number" });
    }
    ...
    if (formData.users.length === 0 || formData.users.length > 2) {
      return res
        .status(400)
        .json({ errormessage: "Invalid number of assigned users" });
    }
...
const putList = async (req, res, next) => {
  try {
    ...
    if (formData.checkList) {
      updateQuery = {
        checkList: { ...listDetails.checkList, ...formData.checkList },
      };
    } else {
      updateQuery = { status: formData.status };
    }
    const updatedList = await ProManage.findByIdAndUpdate(listId, updateQuery, {
      new: true,
    });
...
const getAnalytics = async (req, res, next) => {
  try {
    const allData = await ProManage.find(
      {},
      { priority: 1, status: 1, _id: 0 }
    );
    if (!allData) {
