...
export default function List(props) {
  ...
  const handleCheckbox = async (listId, listKey, checkList) => {
    const formData = { checkList: { [listKey]: checkList === 'yes' ? 'no' : 'yes' } };
    ...
    <>
      ...
      <div className={styles.status}>
        ...
        <div className={styles.scrolly}>
          ...
          {list && list.response && <div className={styles.response}>{list.response}</div>}
          {list &&
            Array.isArray(list) &&
            list.map((task, index) => (
              <div key={task._id} className={styles.task}>
                ...
                {Object.keys(task.list).length > 0 && (
                  ...
                  <img
                    ...
                    style={{ display: toggleArrow[index.toString()] ? 'none' : 'flex' }}
                    src={ArrowUp}
                    alt="arrow-up"
                  />
                  <img
                    onClick={() => handleToggleArrow(index.toString())}
                    style={{ display: toggleArrow[index.toString()] ? 'flex' : 'none' }}
                    src={ArrowDown}
                    alt="arrow-down"
                  />
                </div>
                {Object.keys(task.list).map((key) => (
                  ...
                  >
                    <label className={styles.container}>
                      <input type="checkbox" onChange={() => handleCheckbox(task._id, key, task.checkList[key])} checked={task.checkList[key] === 'yes'} />
                ...
                <p style={{ backgroundColor: task.status === "Done" ? "#63C05B" : task.status !== "Done" && isPastDate(task.date) ? "#CF3636" : null, color: task.status === "Done" ? "white" : task.status !== "Done" && isPastDate(task.date) ? "white" : null }}>
                  ...
                  <span>
                    {DEFAULT_STATUS.map((button, idx) => (
                      <button
                        key={idx}
                        onClick={() => { handleButtonClick(task._id, button) }}
                        style={{ display: props.index !== idx ? 'flex' : 'none' }}
                      >
                        {button}
                      </button>
                    ))}
                ...
                <div style={{ display: activeTaskId === task._id ? 'block' : 'none' }} className={styles.action}>
  ...
  state: {
    ...
    edit: true,
                  ...
                  <div onClick={() =>  task.userId === userId ? navigate(`/delete/${task._id}`): toast.warning("Only Admin can delete")} style={{ color: '#CF3636' }}>
                    Delete
                  </div>
                </div>
